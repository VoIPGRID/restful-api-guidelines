...
* You must design your APIs consistently with these guidelines; use of a
  linter for automated rule checks.
* You must call for early review feedback from peers and client developers for
all component external APIs, i.e. all apis with `x-api-audience =/= component-internal` (see <<219, API Audience>>).
...
We focus on resource oriented HTTP/REST API style (and related tooling and infrastructure support) for general purpose
...
* `https://github.com/zalando/restful-api-guidelines` {dash} used to refer
  to user defined, immutable API specification revisions published via the
  internal API repository.
* `https://github.com/zalando/restful-api-guidelines/{model.yaml}` {dash}
