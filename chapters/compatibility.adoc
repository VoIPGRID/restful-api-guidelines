...
`application/vendor.cart+json;version=2`. For incompatible changes, a new
...
* `custom-media-type` is a custom type name, e.g. `vendor.cart`
...
* Use a custom media type, e.g. `application/vendor.cart+json`
...
TIP: You {SHOULD} avoid media type versioning. Until an incompatible change is necessary, it is recommended to stay
